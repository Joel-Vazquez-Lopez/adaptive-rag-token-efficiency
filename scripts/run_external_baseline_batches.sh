...
if [[ -z "$DATASET" ]]; then
  echo "Usage: scripts/run_external_baseline_batches.sh {scifact|bioasq|hotpotqa|asqa|msmarco|all} [total]"
  exit 1
fi
...
try:
    ...
    raise SystemExit(
        "Missing LLMLingua. Install it first with: python -m pip install llmlingua"
    )
PY
...
run_one() {
  ...
  local output_root="outputs/external_baselines/${dataset}_llama70b_${TOTAL}"
  ...
  python scripts/run_llm_batches.py \
    ...
    --methods fixed_10 adaptive_k_official flare_lite llmlingua2_top10 llmlingua2_adaptive_k_official answer_aware_fallback task_aware_coverage_ultra \
